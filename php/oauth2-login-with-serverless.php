...
echo "(Attempt to access) Private API call\n";
$response = http("https://31f5ff35.eu-gb.apigw.appdomain.cloud/private-authtest/Hello", false);

var_dump($response);

// From our new backend we get the information required to send the user to the 
// App ID server
$appAuth = http("https://31f5ff35.eu-gb.apigw.appdomain.cloud/authtest/GetApplicationEndpoint");
var_dump($appAuth);

// These client details should not be kept on a client machine 
// as they are easily compromised.
// How then can we do this?
// $client_id = '255d5697-8bdb-4041-9480-1558d4e1dd47';
//$client_secret = 'N2RhYTRjZTUtYjBmYy00ZTE1LWFiNjEtMjZiYTA4YjMxZTY5';
$metadata = http($appAuth->openid);
...
// this can be anything on "localhost:3000" not that setting it to
// 127.0.0.1 and not localhost won't work as it is the exact string that is checked.
...
// again we use a backend url to protect our client_secret
$response = http("https://31f5ff35.eu-gb.apigw.appdomain.cloud/authtest/GetAccessToken", [
...
$response = http("https://31f5ff35.eu-gb.apigw.appdomain.cloud/authtest/IsValidToken", [
    'token' => $access_token,
    'client_id' => $appAuth->clientId
]);
